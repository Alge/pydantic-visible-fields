...
class PaginatedResponse(BaseModel, Generic[T]):
    ...
    # Allow arbitrary types in the 'data' list, as they might be processed response models
...
async def from_async_iterable(
    ...
    Returns:
        A PaginatedResponse object containing the processed data and pagination metadata.
    ...
    logger.debug(
        f"from_async_iterable called: limit={limit}, offset={offset}, role='{actual_role}', effective_limit={effective_limit}"
    )
    ...
    if effective_limit > 0:
        ...
        try:
            async for item in iterator:
                items_fetched += 1
                logger.debug(f"Fetched async item {items_fetched}/{fetch_target}. Processing...")
                ...
                if items_fetched == fetch_target:
                    ...
                    has_more = True
                    logger.debug(f"Reached fetch target ({fetch_target}), setting has_more=True.")
        ...
        logger.debug(f"Finished fetching. Fetched {items_fetched} items. "
    ...
    logger.debug(f"Calculated next_offset={next_offset} (offset={offset} + limit={limit})")
...
def from_iterable(
    ...
    logger.debug(
        f"from_iterable called: limit={limit}, offset={offset}, role='{actual_role}', effective_limit={effective_limit}"
    )
        ...
        try:
            while fetch_count < effective_limit:
                try:
                    item = next(iterator)
                    fetch_count += 1
                    logger.debug(f"Processing sync item {fetch_count}/{effective_limit}...")
                    response_item = visible_fields_response(item, actual_role)
            ...
            if processed_count == effective_limit:
                try:
                    ...
                    logger.debug("Successfully fetched one extra item, setting has_more=True.")
                except StopIteration:
    ...
    next_offset = offset + limit
    logger.debug(f"Finished processing. Returning {processed_count} items. has_more={has_more}. next_offset={next_offset}")
